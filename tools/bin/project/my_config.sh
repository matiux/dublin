...
phpstan() {
  ./vendor/bin/phpstan analyse \
    -c "$TOOLS_PATH"/phpstan/phpstan.neon \
    "$@"

  local PHPSTAN_STATUS=$?

  if [[ "$INTERNAL_BADGE_CREATION" == true ]]; then
    if [[ "$PHPSTAN_STATUS" -eq 0 ]]; then
      create_badge "static analysis" passed phpstan
    else
      create_badge "static analysis" failed phpstan
    fi
  fi

  return $PHPSTAN_STATUS
}
